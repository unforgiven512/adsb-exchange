...
if ! command -v git &>/dev/null || ! command -v wget &>/dev/null || ! command -v unzip &>/dev/null; then
	if command -v apt-get &>/dev/null; then
		apt-get update || true
		apt-get install -y --no-install-recommends --no-install-suggests git wget unzip || true
	elif command -v pacman &>/dev/null; then
		pacman -Sy || true
		pacman -S --noconfirm git wget unzip || true
	else
		echo -e "\033[1m--- ERROR: unsupported package manager... ---\033[0m\n\n" 1>&2
		return 1
	fi
fi
